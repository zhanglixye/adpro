...
const STATUSES = [
    { text: Vue.i18n.translate('order.order_details.search_condition.status.active'), value: _const.FLG.ACTIVE },
    { text: Vue.i18n.translate('order.order_details.search_condition.status.archive'), value: _const.FLG.INACTIVE },
]
const initialState = {
    processingData: {
        orderDetails: [],
    ...
    orderFileImportColumnConfigPrefix: 'order_file_import_column_configs_',
...
const store = new Vuex.Store({
    ...
    actions: {
        async searchOrderDetailList({ commit, getters, state }) {
            ...
            // TODO:検索API用のパラメータ設定（配列形式に変換）。一時的に追加したものなので後で削除
            if (Array.isArray(searchParams.status)) {
                searchParams.status = searchParams.status.map((status) => status.value)
            } else if (typeof searchParams.status === 'number') {
            ...
            try {
                ...
                // TODO:検索画面表示用のパラメータ設定（v-combobox対応に選択済み表示対応）。一時的に追加したものなので後で削除
                state.searchParams.status = state.statuses.filter((status) => searchParams.status.includes(status.value))
        ...
        async deleteOrderDetails({ commit, state }) {
            commit('setLoading', true)
            const searchParams = Vue.util.extend({}, {
                order_id: state.searchParams.order_id,
                order_detail_ids: state.processingData.selectedOrderDetailIds,
            ...
            try {
                const res = await axios.post('/api/order/order_details/bulk_delete', searchParams)
                // 検索条件をstoreに保存
                if (res.data.status !== 200) throw res.data.message
                commit('setLoading', false)
            } catch (error) {
    ...
    getters: {
        initialHeaders: (state, getters) => {
            const displayItemNumber = state.processingData.orderFileImportColumnConfigs.length
            return {
