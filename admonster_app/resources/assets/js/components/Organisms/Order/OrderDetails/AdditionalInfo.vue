    ...
    <div class="elevation-1">
        ...
        >
            ...
            <v-card v-if="isEdit">
                ...
                <v-card-text>
                    <v-text-field
                        v-model="inputAdditional.title"
                        :label="$t('order.order_details.show.information_component_management.additional_info.title_label')"
                    >
                    </v-text-field>
                    <v-textarea
                ...
                <v-card-actions class="justify-center">
                    ...
                    <v-btn color="primary" @click="save" :disabled="validateFlg">{{ $t('common.button.save') }}</v-btn>
                            ...
                            <div v-for="(item, index) in additionalInfos" :key="item.id" class="add-info-list pa-1">
                                ...
                                >
                                    ...
                                    <div class="header-summary ml-2" >
                                        <span class="ml-1 body-1 font-weight-bold">{{ user_name(item.updated_user_id) }}</span>
                                        <span v-if="isUpdate(item)" class="ml-1 body-1 font-weight-bold">
                                        ...
                                        <span v-show="!isOpenToggle(item.id)">
                                            <span class="ml-2 body-1 font-weight-bold">{{ item.title }}</span>
                                            <span class="caption">
                                    ...
                                    <v-spacer></v-spacer>
                                    <div class="ml-1" style="min-width:100px" v-show="!isOpenToggle(item.id)">
                                ...
                                <div v-show="isOpenToggle(item.id)" class="pa-2 pl-5 text-left">
                                    <div style="display: flex;" class="mb-3">
                                        ...
                                        <v-spacer></v-spacer>
                                        <span v-if="item.created_at" style="white-space: nowrap;">
                                            {{ item.recieved_at | formatDateYmdHm }}
                                        </span>
                                    </div>
...
export default {
    ...
    computed: {
        validateFlg () {
            if (this.inputAdditional.title != '' || this.inputAdditional.content != '' || this.uploadFiles.length > 0){
                return false
            } else return true
        },
        orderDetailData () {
        ...
        async storeAdditionalInfo () {
            ...
            try {
                ...
                const res = await axios.post('/api/order/additional_infos/store', {
                    ...
                    title: this.inputAdditional.title,
