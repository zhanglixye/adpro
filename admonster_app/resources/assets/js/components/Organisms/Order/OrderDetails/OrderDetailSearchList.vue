    ...
    <div id="search-list">
        ...
        <v-layout row wrap align-center>
            ...
            <v-tooltip top style="max-width: 40px;" v-if="isAdmin">
                ...
                <span>{{ $t('order.order_details.list.to_archive') }}</span>
            </v-tooltip>
            <v-tooltip top style="max-width: 40px;" v-if="isAdmin">
                <v-btn
                    flat
                ...
                <span>{{ $t('order.order_details.list.to_deleted') }}</span>
            </v-tooltip>
            ...
            <v-tooltip top style="max-width: 40px;" v-if="isAdmin">
                <v-btn
                    ...
                    slot="activator"
                    v-if="selectedOrderDetailIds.length !== 0"
                    @click="openBulkUpdateOrderDetailDialog()"
                >
                    <v-icon>mdi-pencil-box-multiple</v-icon>
                </v-btn>
                <span>{{ $t('order.order_details.list.bulk_update_order_detail') }}</span>
            </v-tooltip>
        ...
        <div class="data-content">
                    ...
                    >
                        ...
                        <template v-for="header in shownHeaders">
                            ...
                            <td
                                v-else
                                :key="header.value"
                                :class="getAlignClassName(header.value)"
                            >
                                <v-tooltip top>
            ...
            <!--pagination-->
            <progress-circular v-if="loading" style="z-index: 200;"></progress-circular>
        </div>
        ...
        <bulk-update-order-detail-dialog ref="bulkUpdateOrderDetailDialog"></bulk-update-order-detail-dialog>
...
import importedSettingStore from '../../../../stores/Admin/ImportedFiles/ImportedSetting/store'
import BusinessSelectionDialog from './BusinessSelectionDialog'
import BulkUpdateOrderDetailDialog from './BulkUpdateOrderDetailDialog'
...
export default {
    ...
    watch: {
        ...
        async isToImportSettingPage(val) {
            if (val) {
                const formData = Vue.util.extend({}, {
                    file_name: importedFilesStore.state.uploadFile.tmpFileInfo.file_name,
                    file_path: importedFilesStore.state.uploadFile.tmpFileInfo.file_path,
                    folder_name: 'temporary_files'
                })
                try {
                    const res = await axios.post('/api/utilities/upload_from_temporary_file_to_s3', formData)
                    // 検索条件をstoreに保存
                    if (res.data.status !== 200) throw 'Failed to upload the file'
                    importedSettingStore.commit('setFilePath', res.data.file_path)
                    this.formSubmit()
                } catch (error) {
                    console.log(error)
    ...
    methods: {
        ...
        getAlignClassName (value) {
            const orderFileImportColumnConfig = this.orderFileImportColumnConfigs.find(
                item => item.column === value
            )
            if (orderFileImportColumnConfig === undefined) return 'text-xs-center'

            for (const [displayFormatKey, displayFormatValue] of Object.entries(_const.DISPLAY_FORMAT)) {
                if (!displayFormatKey.match(/COMMON_ALIGN/g)) continue
                if (orderFileImportColumnConfig.display_formats.includes(displayFormatValue)) {
                    const align = displayFormatKey.replace('COMMON_ALIGN_', '')
                    return 'text-xs-' + align.toLowerCase() + ' overflow'
                }
            }
            if ([_const.ITEM_TYPE.NUM.ID, _const.ITEM_TYPE.DATE.ID].includes(orderFileImportColumnConfig.item_type)) {
                return 'text-xs-center'
            }
            return 'text-xs-left overflow'
        },
        ...
        openSettingDisplayFormat (element) {
            ...
            for (const [displayFormatKey, displayFormatValue] of Object.entries(_const.DISPLAY_FORMAT)) {
                ...
                if (!['COMMON', itemTypeKey].includes(displayFormatKey.substr(0, end))) continue
                if (displayFormatKey.match(/COMMON_ALIGN/g)) {
                    alignItems.push(
                        {
                            'id': displayFormatValue,
                            'text': this.$t(`order.order_details.dialog.setting_display_format.item_type_common.${_const.PREFIX}${displayFormatValue}`)
                        }
                    )
                } else {
                    displayFormats.push(
        ...
        async deleteOrderDetail () {
            ...
            if (await this.$refs.confirmCheckDialog.show(messages)) {
                try {
                    await store.dispatch('deleteOrderDetails')
                    ...
                    if (error === 'no_admin_permission') {
                        this.$refs.alert.show(this.$t('common.message.no_admin_permission'))
                    } else if (error === 'deleted_data') {
                        this.$refs.alert.show(this.$t('order.order_details.dialog.status.to_deleted.failure'))
                    } else if (error === 'updated_by_others') {
                        this.$refs.alert.show(this.$t('common.message.updated_by_others'))
                    } else {
                        this.$refs.alert.show(this.$t('common.message.internal_error'))
        ...
        async changeStatus (status) {
            ...
            if (status === _const.FLG.INACTIVE) message = this.$t('order.order_details.dialog.status.to_archive.text')
            if (await this.$refs.confirm.show(message)) {
                try {
                    await store.dispatch('updateOrderDetails', { status: status })
                    await this.$refs.alert.show(this.$t('common.message.updated'))
                    await store.dispatch('searchOrderDetailList')
                } catch (error) {
                    console.log('error: ', error)
                    if (error === 'no_admin_permission') {
        ...
        displayStatusName (status) {
            if (status === _const.FLG.ACTIVE) {
                ...
                // アーカイブ
                return this.$t('order.order_details.list.status.archive')
            }
        },
        ...
        openBusinessSelectionDialog: async function() {
            ...
            if (businessId) {
                ...
                if (await this.$refs.confirmCheckDialog.show([this.$t('order.order_details.dialog.request_bulk_creation.confirm')])) {
                    ...
                    this.selectedOrderDetailIds.forEach(orderDetailId => {
                        ...
                        if ([12, 14].indexOf(businessId) !== -1) {
                            ...
                            let columnsText = this.formatColumnsForDisplay(this.selectedOrderDetailColumns[orderDetailId], filteringColumns)
                            body = 'ご担当者様\n\nお疲れ様です。\n下記、Xone紐づけをお願いします。\n\n' + columnsText
                        } else if ([16].indexOf(businessId) !== -1) {
                            let filteringColumns = ['カテゴリ', 'アカウント名', '最新取得配信日時(メッセージ)']
                            let columnsText = this.formatColumnsForDisplay(this.selectedOrderDetailColumns[orderDetailId], filteringColumns)
                            body = columnsText
                        } else if ([17].indexOf(businessId) !== -1) {
                            let filteringColumns = ['カテゴリ', 'アカウント名', '最新取得配信日時(タイムライン)']
                            let columnsText = this.formatColumnsForDisplay(this.selectedOrderDetailColumns[orderDetailId], filteringColumns)
                            body = columnsText
                        } else {
        ...
        openBulkUpdateOrderDetailDialog: async function() {
            let updateData = await this.$refs.bulkUpdateOrderDetailDialog.show()
            if (updateData) {
                if (await this.$refs.confirm.show(this.$t('order.order_details.dialog.bulk_update_order_detail.confirm'))) {
                    this.invalidClickDialog = true
                    store.commit('setLoading', true)
                    try {
                        const formData = new FormData()
                        formData.append('order_detail_ids', JSON.stringify(this.selectedOrderDetailIds))
                        formData.append('order_detail_columns', JSON.stringify(updateData['columns']))
                        formData.append('custom_statuses', JSON.stringify(updateData['customStatuses']))
                        formData.append('started_at', updateData['startedAt'])
                        const res = await axios.post('/api/order/order_details/bulk_update_order_detail', formData)
                        this.invalidClickDialog = false
                        if (res.data.status !== 200) throw res.data.message
                        this.$refs.alert.show(
                            this.$t('order.order_details.dialog.bulk_update_order_detail.success'),
                            () => store.dispatch('searchOrderDetailList')
                        )
                    } catch (error) {
                        this.invalidClickDialog = false
                        ...
                        if (error === 'no_admin_permission') {
                            ...
                            this.$refs.alert.show(this.$t('order.order_details.dialog.bulk_update_order_detail.failure'))
                        }
                    }
                    store.commit('setLoading', false)
                }
...
#search-list >>> .theme--light.v-table thead th:not(:last-child),
#search-list >>> .theme--light.v-table tbody td:not(:last-child) {
