...
import OrderDetailSystemItems from './OrderDetailSystemItems'
...
export default {
    ...
    data() {
        return {
            ...
            contents: [
                ...
                {
                    id: 7,
                    component: 'orderDetailSystemItems',
                    show: true,
                },
            ],
        }
    ...
    async created() {
        ...
        // 案件明細を作成する場合、案件情報を編集モードで表示
        if (this.orderDetailId === 0) this.toEditMode(this.contents.find(content => content.component === 'orderDetailInfo').id)
    },
    methods: {
        isArrowReverse(id) {
        ...
        mode: function(id) {
            if (!this.isAdmin || this.isDeleted) return _const.DISPLAY_MODE.READ_ONLY
