        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <div style="max-height: 66vh; overflow-y: auto; margin-top: 16px">
                    ...
                    >
                        <v-radio-group
                            v-if="displayFormat.type === 'radio'"
                            v-model="selectedAlignValue"
                            row
                        >
                            <v-radio
                                v-for="(item) in displayFormat.items"
                                :key="item.id"
                                :label="item.text"
                                :value="item.id"
                            ></v-radio>
                        ...
                        <v-checkbox
                            ...
                            :input-value="selectedDisplayFormats.includes(displayFormat.id)"
                            :key="displayFormat.id"
                        ></v-checkbox>
                    </span>
                    <span
    ...
    methods: {
        show (itemName, displayFormats, selectedDisplayFormats, orderFileImportColumnConfigId, itemType) {
            ...
            displayFormats.forEach(displayFormat => {
                if (displayFormat.type === 'radio') {
                    this.selectedAlignValue = null
                    displayFormat.items.forEach(item => {
                        if (selectedDisplayFormats.includes(item.id)) {
                            this.selectedAlignValue = item.id
                            this.selectedDisplayFormats = this.selectedDisplayFormats.filter(selectFormat => selectFormat !== item.id)
                        }
                    })
                }
            })
        ...
        save () {
            let selectedDisplayFormats = this.selectedAlignValue === null ? [] : [this.selectedAlignValue]
            selectedDisplayFormats = selectedDisplayFormats.concat(this.selectedDisplayFormats)
            eventHub.$emit(this.callback, selectedDisplayFormats, this.orderFileImportColumnConfigId)
            this.close()
        },
    }
}
