        ...
        >
            ...
            <div class="row_new ma-0 mt-3 bottom12_button">
                <div class="btn-icon">
                    ...
                    <checkbox-dialog :loadingDisplay="loadingDisplay" :checkboxListMade="!madeAG" v-show="checkboxShow" class="checkbox-style" :ajaxPath="ajaxPath" :axiosFormData="axiosFormData" ref="checkboxDialog"></checkbox-dialog>
...
export default {
    ...
    created(){
        const self = this;
        //默认work_time赋值
        axios.post(this.ajaxPath+'getWorktime',this.axiosFormData).then(workTimeResult => {
            if (workTimeResult.data.result == 'success'){
                ...
                if (time_work_new.started_at == null) {
                    ...
                    //let started_time = this.convertToLocalTime(time_work_new.started_at,'YYYY-MM-DD HH:mm:ss',time_work_new.timezone);
                ...
                if (time_work_new.finished_at == null) {
                    ...
                    //let finished_time = this.convertToLocalTime(time_work_new.finished_at,'YYYY-MM-DD HH:mm:ss',time_work_new.timezone);
    ...
    methods:{
        ...
        async getEmail(checkboxDateReview){
            ...
            getEmailData.iHour = this.$refs.dateBuss.$data.savedRealTime.user_timekeeping;
            getEmailData.beginDateTime = this.$refs.dateBuss.$data.user_start_date===''?null:(this.$refs.dateBuss.$data.user_start_date + ' '+ this.$refs.dateBuss.$data.user_start_time);
            getEmailData.endTime = this.$refs.dateBuss.$data.user_end_date===''?null:(this.$refs.dateBuss.$data.user_end_date + ' '+ this.$refs.dateBuss.$data.user_end_time);
            getEmailData.remark = this.$refs.dateBuss.$data.remark;
        ...
        getSaveData(){
            ...
            G00000_27.G00000_35 = {
                // 'C00700_36':this.$refs.dateBuss.$data.user_start_date===''?null:this.convertToTime(this.$refs.dateBuss.$data.user_start_date + ' '+ this.$refs.dateBuss.$data.user_start_time),
                // 'C00700_37':this.$refs.dateBuss.$data.user_end_date===''?null:this.convertToTime(this.$refs.dateBuss.$data.user_end_date + ' ' + this.$refs.dateBuss.$data.user_end_time),
                'C00700_36':this.$refs.dateBuss.$data.user_start_date===''?null:this.$refs.dateBuss.$data.user_start_date + ' '+ this.$refs.dateBuss.$data.user_start_time+':00',
                'C00700_37':this.$refs.dateBuss.$data.user_end_date===''?null:this.$refs.dateBuss.$data.user_end_date + ' ' + this.$refs.dateBuss.$data.user_end_time+':00',
                'C00100_38':Number(this.$refs.dateBuss.$data.user_timekeeping/60).toFixed(2),
                'C00100_39':this.$refs.dateBuss.$data.remark,
            }
            //file
            let fileUp = [];
        ...
        convertToLocalTime: function (utcDate, outPutFormat,timezone) {
            if (utcDate == ''){
                return '';
            }
            return  moment.tz(utcDate,timezone).format(outPutFormat);
        },
