    ...
    <v-card class="card_t">
        <template>
            ...
            >
                ...
                <div class="mt-3 justify-space-between" style="display: flex">
                    <div>
                    ...
                    <date-buss-t :disabled_state="statusBool" ref="dateBuss" style="display: inline-block"></date-buss-t>
                </div>
            </v-form>
        </template>
        <div class="unKnowDeal" v-show="unKnowStatus">
            <div>下記原因で不明アリ"で処理されました。</div>
            <div>{{unKnowData}}</div>
        </div>
        ...
        async save(flag){
            try {
                ...
                let outInputData = {
                    ...
                    work_time:{
                        started_at:this.$refs.dateBuss.$data.user_start_date===''?null:this.$refs.dateBuss.$data.user_start_date + ' '+ this.$refs.dateBuss.$data.user_start_time+':00',
                        finished_at:this.$refs.dateBuss.$data.user_end_date===''?null:this.$refs.dateBuss.$data.user_end_date + ' ' + this.$refs.dateBuss.$data.user_end_time+':00',
                        total:Number(this.$refs.dateBuss.$data.user_timekeeping/60).toFixed(2),
                        remark:this.$refs.dateBuss.$data.remark,
                    }
                };
