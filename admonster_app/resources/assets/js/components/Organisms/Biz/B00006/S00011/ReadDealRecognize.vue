...
<!--                            <a @click="download(item.file_seq_no,item.name)">{{item.name}}</a>-->
                ...
                <v-card-text>
                    <div v-for="(item_parent,i) in checkList" :key="i" :class="[checkboxListMade?'check_dia_border':'check_dia_border_none',checkbox_title_border,(item_parent.pos == 1)?'ml-au30':'']">
                        ...
                        <v-flex :class="[!checkboxListMade?'flex-wrap':'']">
                            <component
                                    disabled
                                    :is="diffComponentShow(item)"
                                    v-show="diffComponentShow(item) !== null"
                                    :items="item.items"
                                    item-text="content"
                                    item-value="value"
                                    v-model="item.value"
                                    ...
                                    :selectFiles="uploadFilesCheckbox"
                            ></component>
                        </v-flex>
                ...
                <v-card-text>
                    <span style="line-height: 33px">
                        作業時間  <span style="font-size: 12px;cursor: default;">{{data==null?parseFloat(isHour):parseFloat(data.iHour)}}</span>  M
                    </span>
                    <div style="line-height: 33px">
                        備考:  <span style="font-size: 12px;cursor: default;">{{data==null?'':data.remark}}</span>
                    </div>
                </v-card-text>
...
import AExtend from '../../Common/DynamicPage/AExtend';
import VRadioExtend from '../../Common/DynamicPage/VRadioExtend';
import VDatePickerExtend from '../../Common/DynamicPage/VDatePickerExtend';
import FileUpload from '../../../../Atoms/Biz/Common/FileUpload';
import FilleUploadExtend from '../../Common/DynamicPage/FilleUploadExtend';
export default {
    components: {
        FilleUploadExtend,
        FileUpload,
        VDatePickerExtend, VRadioExtend, AExtend, AlertDialog,NotifyModal,ProgressCircular,FilePreviewDialog},
    props:{
        data:Object,
    ...
    watch:{
        data(value){
            ...
            this.getFileInformationRead();
            if (isNaN(parseFloat(value.iHour))){
                //重新调用workTime接口
                axios.post('/api/biz/common/mail/getWorktime',this.axiosFormData).then(workTimeResult => {
                    if (workTimeResult.data.result == 'success'){
                        let time_work =  workTimeResult.data.data;
                        value.beginDateTime = time_work.started_at.substring(0,time_work.started_at.length-3);
                        value.endTime = time_work.finished_at.substring(0,time_work.finished_at.length-3);
                        value.iHour = (time_work.work_time*60).toFixed(0);
                    }
                });
            }
        }
    ...
    methods:{
        ...
        diffComponentShow(item){
            if (item.component_type == 0 || item.component_type == 400){
                ...
                return 'fille-upload-extend';
            } else {
                return null;
            }
        },
        async getFileInformationRead() {
            try {
                let formData = new FormData();
                let seq_no_array;
                //将seq_arr作为参数
                this.data.checkboxDialog.forEach((item) => {
                ...
                if (res.data.result == 'success'){
                    ...
                    this.uploadFilesCheckbox = [];
                    for (const value of contentFile){
                        this.uploadFilesCheckbox.push({
