    ...
    <div id="mail">
        ...
        >
            ...
            <div class="pa-2">
                ...
                <div class="pa-2">
                    <template v-for="item in beforeWorkItemConfigs">
                        <v-layout row wrap :key="item['item_key']">
                            ...
                            <v-flex xs8 class="height-center">
                                <template v-if="isItemTypeFile(item.item_type)">
                                    <v-layout row wrap>
                                        <template v-for="seq in convertToArray($getValue(beforeDelivery, item['item_key']))">
                                ...
                                <template v-else>
                                    <span v-text="$getValue(beforeDelivery, item['item_key'])"></span>
                                </template>
                            </v-flex>
                        </v-layout>
                    </template>
    ...
    computed: {
        ...
        attachments () {
            ...
            if (this.deliveryFiles) {
                let fileSeqNumbers = []
                this.beforeWorkItemConfigs.forEach(item => {
                    if (this.isItemTypeFile(item['item_type'])) {
                        fileSeqNumbers = [...fileSeqNumbers, ...this.convertToArray(this.$getValue(this.beforeDelivery, item['item_key']))];
                    }
                })
        ...
        beforeWorkItemConfigs () {
            return this.beforeWork ? this.beforeWork['item_configs'] : []
        }
    },
    methods: {
        download (attachment) {
        ...
        isItemTypeFile (itemType) {
            return [_const.ITEM_CONFIG_TYPE.FILE_INPUT, _const.ITEM_CONFIG_TYPE.SINGLE_FILE_INPUT].includes(itemType)
        },
        convertToArray (element) {
            return element ? [element].flat() : []
        },
