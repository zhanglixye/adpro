                                    ...
                                    >
                                        ...
                                        <template v-for="(item, i) in items">
                                            <tr :key="i">
                                                ...
                                                <url-data-entry-rule-dialog :readonly="readonly" @setting="setting" ref="urlDataEntryRuleDialog"></url-data-entry-rule-dialog>
                                            </tr>
                                        </template>
                                    </table>
...
import UrlDataEntryRuleDialog from '../../../Organisms/Admin/ImportedFiles/UrlDataEntryRuleDialog'
...
export default {
    ...
    data: () => ({
        ...
        dataTypeCandidates: [
            ...
            {
                label: Vue.i18n.translate('imported_files.order_imported_setting.url_data_entry_rule.subject'),
    ...
    watch: {
        items: {
            ...
            async handler() {
                ...
                // ADPORTER件名が未選択の場合は自動で3つチェックを入れる
                if (this.subjects.length === 0) {
                    this.subjects = this.items.slice(0, 3).map(item => item['row'])
                }
            },
        },
    ...
    methods: {
        ...
        async deleteTmpFileInfo () {
            ...
            try {
                ...
                const res = await axios.post('/api/utilities/deleted_s3_file', {
                    file_path: this.filePath
                })
                if (res.data.status !== 200) throw 'Failed to deleted the file'
            } catch (e) {
                console.log(e)
            }
            this.loading = false
        ...
        async getSettingInformation () {
            try {
                ...
                for (const key in res.data.setting_rules) {
                    this.setting(
                        {
                            inputRule: JSON.parse(res.data.setting_rules[key]['rule']),
                            displayRule: res.data.setting_rules[key]['display_format'],
                            selectType: this.items[key]['dataType'],
                        }, key)
                }
        ...
        formSubmit () {
            ...
            // データ形式に合わせて設定
            const settingRules = JSON.parse(JSON.stringify(store.state.settingRules))
            for (const key in this.items) {
                const item = this.items[key]

                // 設定されている場合
                if ((key in settingRules)) {
                    const settingRule = settingRules[key]

                    // settingRulesに入っている内容と選択されているデータ形式の確認
                    if (
                        (![undefined, null].includes(settingRule) && typeof settingRule === 'object')
                        && ('selectType' in settingRule && settingRule.selectType !== item.dataType)
                    ) { // 設定した内容とデータ形式が異なる場合
                        // データ形式とsettingRuleが異なる場合はダイアログからデータを取得
                        let form = null
                        switch (item.dataType) {
                        case _const.ITEM_TYPE.STRING.ID:
                            form = this.$refs.textDataEntryRuleDialog[key].form
                            break
                        case _const.ITEM_TYPE.URL.ID:
                            form = this.$refs.urlDataEntryRuleDialog[key].form
                            break
                        case _const.ITEM_TYPE.NUM.ID:
                            form = this.$refs.numberDataEntryRuleDialog[key].form
                            break
                        case _const.ITEM_TYPE.DATE.ID:
                            form = this.$refs.dateDataEntryRuleDialog[key].form
                            break
                        default:
                            break
                        }
                    ...
                    case _const.ITEM_TYPE.STRING.ID:
                    case _const.ITEM_TYPE.URL.ID:
                        displayRule.push(_const.DISPLAY_FORMAT.COMMON_ALIGN_LEFT)
                        break
                    case _const.ITEM_TYPE.NUM.ID:
                        displayRule.push(_const.DISPLAY_FORMAT.COMMON_ALIGN_RIGHT)
                        break
                    case _const.ITEM_TYPE.DATE.ID:
                        displayRule.push(_const.DISPLAY_FORMAT.COMMON_ALIGN_CENTER)
                        break
                    default:
                        break
                    }
                    this.setting(
                        {
                            inputRule: null,
                            displayRule: displayRule,// 各データタイプに合わせて設定
                            selectType: item.dataType
                        },
                        key
                    )
                }
        ...
        openDataEntryRuleDialog (dataType, i) {
            ...
            case this.itemTypeDateId:
                ...
                break
            case this.itemTypeUrlId:
                this.$refs.urlDataEntryRuleDialog[i].show(i, store.state.settingRules[i])
                break
            }
        },
        filePreview: async function (type = '') {
            const file = { file_path: this.filePath, name: this.inputs.tmp_file_info.file_name }
