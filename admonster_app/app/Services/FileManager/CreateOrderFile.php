    ...
    public static function create(array $order_detail_ids): array
    {
        ...
        foreach ($order_ids as $order_id) {
            ...
            $custom_statuses = DB::table('custom_statuses')
                ->join('labels', 'custom_statuses.label_id', '=', 'labels.label_id')
            ...
            $custom_status_ids = $custom_statuses->pluck('custom_statuses.id')->toArray();
            ...
            foreach ($order_detail_ids as $order_detail_id) {
                ...
                array_push($row, $query->value('order_details.is_active') ? __('order/order_details.list.status.active') : __('order/order_details.list.status.archive'));
                $custom_status_rows = DB::table('order_details_related_custom_status_attributes')
                    ->select(
                        'order_details_related_custom_status_attributes.custom_status_id AS custom_status_id',
                        'labels.name AS name'
                    )
                    ...
                    ->join('labels', 'custom_status_attributes.label_id', '=', 'labels.label_id')
                    ->join('languages', function ($join) {
                        $join->on('labels.language_id', '=', 'languages.id')
                            ->where('languages.code', 'ja');
                    })
                    ->where('order_details_related_custom_status_attributes.order_detail_id', $order_detail_id)
                    ->whereIn('order_details_related_custom_status_attributes.custom_status_id', $custom_status_ids)
                    ->get()
                    ->keyBy('custom_status_id');
                foreach ($custom_status_ids as $custom_status_id) {
                    $custom_status_value = isset($custom_status_rows[$custom_status_id]) ? $custom_status_rows[$custom_status_id]->name : '';
                    array_push($row, $custom_status_value);
                }

                // 先にvalueで値を取得するとpluckで正しく値取得出来なくなるため後判定
            ...
            $each_column_data_formats = array_merge($each_column_data_formats, array($column_data_format => \Config::get('const.ITEM_TYPE.STRING.ID')));
            $custom_statuses_header_rows = $custom_statuses->pluck('labels.name')->toArray();
            foreach ($custom_statuses_header_rows as $custom_statuses_header_row) {
                ++$column_data_format;
                array_push($header_rows, $custom_statuses_header_row);
                $each_column_data_formats = array_merge($each_column_data_formats, array($column_data_format => \Config::get('const.ITEM_TYPE.STRING.ID')));
            }
