...
class OrderDetailsController extends Controller
{
    ...
    public function index(Request $req): JsonResponse
    {
        ...
        // ステータス
        if (is_array($form['status']) && count($form['status']) > 0) {
            $query->whereIn($base_table . '.is_active', $form['status']);
        }
    ...
    public function bulkUpdate(Request $req): JsonResponse
    {
        ...
        try {
            ...
            $update_order_details = DB::table('order_details')
                ...
                ->get();
            if ($update_order_details->where('is_deleted', false)->count() === 0) {
                throw new ExclusiveException("The data has already been deleted");
            }
            if ($update_order_details->where('updated_at', '>=', $form['started_at'])->count() > 0) {
                throw new ExclusiveException("The order details data was updated by another user");
            }
            $order_detail_ids = $update_order_details->where('is_deleted', false)->pluck('id')->all();
            // 排他制御--------------------------------------

            OrderDetail::whereIn('id', $order_detail_ids)
                ->update([
            ...
            report($e);
            switch ($e->getMessage()) {
                case 'no admin permission':
                    $error_message = 'no_admin_permission';
                    break;
                case 'The data has already been deleted':
                    $error_message = 'deleted_data';
                    break;
                default:
                    $error_message = 'updated_by_others';
            }
            return response()->json([
                'status' => $e->getCode(),
                'error' => get_class($e),
     ...
     * @return JsonResponse
     */
    public function bulkDelete(Request $req): JsonResponse
    {
        $user = \Auth::user();
        ...
        try {
            ...
            $delete_order_details = DB::table('order_details')
                ->whereIn('id', $form['order_detail_ids'])
                ->get();
            if ($delete_order_details->where('is_deleted', false)->count() === 0) {
                throw new ExclusiveException("The data has already been deleted");
            }
            if ($delete_order_details->where('updated_at', '>=', $form['started_at'])->count() > 0) {
                throw new ExclusiveException("The order details data was updated by another user");
            }
            $order_detail_ids = $delete_order_details->where('is_deleted', false)->pluck('id')->all();
            // 排他制御--------------------------------------
            OrderDetail::whereIn('id', $order_detail_ids)
                ->update([
    ...
    public function show(Request $req): JsonResponse
    {
        ...
        try {
            ...
            $order_detail_data = $query->select(
                ...
                'order_details.is_deleted AS is_deleted',
                'order_details.created_at AS created_at'
            )->first();

            // 案件明細追加時は登録内容は空にする
            $order_detail_data->order_is_active = $form['order_detail_id'] === 0 ? '' : $order_detail_data->order_is_active;
            $order_detail_data->order_detail_is_active = $form['order_detail_id'] === 0 ? '' : $order_detail_data->order_detail_is_active;
            $order_detail_data->is_deleted = $form['order_detail_id'] === 0 ? '' : $order_detail_data->is_deleted;
            $order_detail_data->created_at = $form['order_detail_id'] === 0 ? '' : $order_detail_data->created_at;
            ...
            foreach ($order_file_import_column_config_ids as $order_file_import_column_config_id) {
                $column_query = DB::table('order_file_import_column_configs')->select(
                    ...
                    'order_file_import_column_configs.subject_part_no AS subject_part_no'
                )
    ...
    public function save(Request $req): JsonResponse
    {
        ...
        try {
            ...
            if ($form['order_detail_id'] === 0) {
                ...
                $order_detail->is_active = $form['order_details_data']['order_detail_is_active'];
                ...
                // 排他制御--------------------------------------
                $update_order_detail = DB::table('order_details')
                    ->where('id', $form['order_detail_id']);
                if ($update_order_detail->value('is_deleted')) {
                    throw new ExclusiveException("The data has already been deleted");
                }
                $can_update = OrderDetail::leftJoin('order_detail_values', 'order_details.id', '=', 'order_detail_values.order_detail_id')
                    ->where('order_details.id', $form['order_detail_id'])
                    ->where(function ($query) use ($form) {
                        $query->where('order_details.updated_at', '>=', $form['started_at'])
                            ->orWhere('order_detail_values.updated_at', '>=', $form['started_at']);
                    })
                    ->count() === 0;
                if (!$can_update) {
                    throw new ExclusiveException("The data was updated by another user");
                }
                $order_id = $update_order_detail->value('order_id');
                ...
                OrderDetail::where('id', $form['order_detail_id'])
                    ->update(
                        [
                            ...
                            'is_active' => $form['order_details_data']['order_detail_is_active'],
                            'updated_at' => $register_time,
                            'updated_user_id' => $user->id
                        ]
                    );
            ...
            $can_update = DB::table('custom_statuses')
                ->leftJoin('custom_status_attributes', 'custom_status_attributes.custom_status_id', '=', 'custom_statuses.id')
                ->whereIn('custom_statuses.id', $custom_status_ids)
                ->where(function ($query) use ($form) {
                    $query->where('custom_statuses.updated_at', '>=', $form['started_at'])
                        ->orWhere('custom_status_attributes.updated_at', '>=', $form['started_at']);
                })
                ->count() === 0;

            $can_update = $can_update ? DB::table('order_details_related_custom_status_attributes')
     ...
     * @return JsonResponse
     */
    public function saveSystemItems(Request $req): JsonResponse
    {
        $form = [
            'order_detail_id' => $req->input('order_detail_id'),
            'subject_data' => $req->input('subject_data'),
            'selected_system_status_id' => $req->input('selected_system_status_id'),
            'started_at' => new Carbon($req->input('started_at')),
            'custom_statuses' => $req->input('custom_statuses') ? json_decode($req->input('custom_statuses'), true) : []
        ];
        $register_time = Carbon::now();
        ...
        try {
            ...
            $can_update = OrderDetail::leftJoin('order_detail_values', 'order_details.id', '=', 'order_detail_values.order_detail_id')
                ->where('order_details.id', $form['order_detail_id'])
                ->where('order_details.updated_at', '>=', $form['started_at'])
                ->count() === 0;
            if (!$can_update) {
                throw new ExclusiveException("The data was updated by another user");
            }
            ...
            OrderDetail::where('id', $form['order_detail_id'])
                ->update(
                    [
                        'name' => $form['subject_data'],
                        'is_active' => $form['selected_system_status_id'],
                        'updated_at' => $register_time,
                        'updated_user_id' => $user->id
                    ]
                );
     ...
     * @return JsonResponse
     */
    public function bulkUpdateOrderDetail(Request $req): JsonResponse
    {
        $form = [
            'order_detail_ids' => $req->input('order_detail_ids') ? json_decode($req->input('order_detail_ids'), true) : [],
            'order_detail_columns' => $req->input('order_detail_columns') ? json_decode($req->input('order_detail_columns'), true) : [],
            'custom_statuses' => $req->input('custom_statuses') ? json_decode($req->input('custom_statuses'), true) : [],
            'started_at' => $req->input('started_at'),
        ];
        $register_time = Carbon::now();
        ...
        try {
            ...
            $can_update = DB::table('order_detail_values')
                ->whereIn('order_detail_id', $order_detail_ids)
                ->where('updated_at', '>=', $form['started_at'])
                ->count() === 0;
            if (!$can_update) {
                throw new ExclusiveException("The data was updated by another user");
            ...
            $order_detail = DB::table('order_details')
                ...
                ->groupBy('order_id');
            if ($order_detail->get()->count() !== 1) {
                throw new ExclusiveException("There are multiple order to update");
            }
            $order_id = $order_detail->value('order_id');
            ...
            // 編集
            foreach ($form['order_detail_columns'] as $column) {
                // OrderDetailValue
                \DB::table('order_detail_values')
                    ->join('order_file_import_column_configs_order_detail_values', 'order_detail_values.id', '=', 'order_file_import_column_configs_order_detail_values.order_detail_value_id')
                    ->where('order_file_import_column_configs_order_detail_values.order_file_import_column_config_id', $column['id'])
            ...
            // 排他制御--------------------------------------
            $custom_status_ids = array_filter(array_column($form['custom_statuses'], 'id'));
            $can_update = DB::table('custom_statuses')
                ->leftJoin('custom_status_attributes', 'custom_status_attributes.custom_status_id', '=', 'custom_statuses.id')
                ->whereIn('custom_statuses.id', $custom_status_ids)
                ->where(function ($query) use ($form) {
    ...
    public function createRequests(Request $req): JsonResponse
    {
        ...
        try {
            ...
            $request_mail->from = $user->name.' <'.$user->email.'>';
    ...
    public function getRelatedMailList(Request $req)
    {
        ...
        foreach ($order_related_mails as $order_related_mail) {
            ...
            if ($order_related_mail->request_mail_id !== null) {// 依頼メール
                $mail = RequestMail::find($order_related_mail->request_mail_id);
                if ($mail === null) {
                    continue;
                }
                $mail->attachments = $mail->requestMailAttachments;
                unset($mail->requestMailAttachments);
            } else {// 送信メール
                $mail = SendMail::find($order_related_mail->send_mail_id);
                if ($mail === null) {
                    continue;
                }
                $mail->load('sendMailAttachments');
                $mail->attachments = $mail->sendMailAttachments;
                unset($mail->sendMailAttachments);
            }

            $mail->original_body = $mail->body;
            $mail->pivot = [
            ...
            if ($mail->content_type === \Config::get('const.CONTENT_TYPE.TEXT')) {
                // 表示はHTMLなので改行コードを<br>タグに変換
                $mail->body = nl2br(e($mail->body));
            }
            // 見出し用に改行を取り除いた前方200文字を保持
            $mail->body_heading = mb_substr(strip_tags($mail->body), 0, 200, 'UTF-8');
            array_push($related_mails, $mail);
        }
        return response()->json([
            'status' => 200,
