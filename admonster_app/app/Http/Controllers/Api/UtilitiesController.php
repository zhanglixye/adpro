...
class UtilitiesController extends Controller
{
    ...
    public function getWorkRequestInfo(Request $req)
    {
        try {
            ...
            // テンプレート取得
            $screen_template = $request_info->step->request_screen_template;
            ...
            if ($before_work_id) {
                ...
                // 作業項目リストを取得
                $before_work['item_configs'] = ItemConfig::where('step_id', $before_work['step_id'])
                    ->selectRaw(
                        'id,'.
                        'step_id,'.
                        'item_key,'.
                    ...
                    ->whereRaw('use_screen>> ? & 1', \Config::get('const.PROCESS_TYPE.DELIVERY'))
                    // 候補から除外する項目/
                    ->orderBy('sort')
                    ->get();
                ...
                $delivery = \DB::table('deliveries')
                    ...
                    ->where('approvals.request_work_id', $before_work_id)
                    ->first();
     ...
     * @return JsonResponse
     */
    public static function uploadFromTemporaryFileToS3(Request $req): JsonResponse
    {
        $form = [
            'tmp_file_info' => $req->input('tmp_file_info'),
        ...
        try {
            // 一時ファイルをS3に保存
            $file_path = $form['folder_name'] . '/' . Carbon::now()->format('Ymd') . '/' . md5(microtime()) . '/' . $form['file_name'];
            $tmp_file = \Storage::drive()->get($form['file_path']);
            Uploader::uploadToS3($tmp_file, $file_path);
     ...
     * @return JsonResponse
     */
    public static function deletedS3File(Request $req): JsonResponse
    {
        try {
            $form = [
                'file_path' => $req->input('file_path'),
