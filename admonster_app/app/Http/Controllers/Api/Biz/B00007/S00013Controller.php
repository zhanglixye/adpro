...
class S00013Controller extends BaseController
{
            ...
            try {
                ...
                foreach ($unziped_file_array as $unziped_file) {
                    ...
                    $upload_file_info = [
                        ...
                        'file_size' => $task_result_file_po->size,
                        'check_file_name' => self::convert($task_result_file_po->name, number_format($task_result_file_po->size))
                    ];
    ...
    public function commitWork(Request $req)
    {
        try {
            ...
            $work_time['started_at'] = MailController::convertUserTimezoneToUtc0(MailController::arrayIfNull($data, ['common_work_time', 'started_at']));
            $work_time['finished_at'] = MailController::convertUserTimezoneToUtc0(MailController::arrayIfNull($data, ['common_work_time', 'finished_at']));
            $work_time['remark'] = MailController::arrayIfNull($data, ['common_work_time','remark']);
