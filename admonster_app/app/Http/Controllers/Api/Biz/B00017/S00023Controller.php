...
namespace App\Http\Controllers\Api\Biz\B00017;
...
class S00023Controller extends BaseController
{
    ...
    public function validationData(array $array, bool $type)
    {
        for ($i = 0; $i < count($array); $i++) {
            ...
            if ($type == true) {
                ...
                if ($array[$i]['C00400_11'] != false) {
                    throw new \Exception(" $row 「いいね！等のリアクションまでを取得しています」を選択しないでください。");
                }
                if ($array[$i]['C00400_12'] != false) {
                    throw new \Exception(" $row 「（動画・画像）マニュアルを確認しました」を選択しないでください。");
                }

                //校验LP下的checkbox
                if ($array[$i]['C00400_13'] != false) {
                    throw new \Exception(" $row 「リンク先のみ取得しました(画像や動画は取得していません)」を選択しないでください。");
                }
                if ($array[$i]['C00400_14'] != false) {
