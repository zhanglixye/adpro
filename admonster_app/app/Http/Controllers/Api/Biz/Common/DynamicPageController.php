...
class DynamicPageController extends BaseController
{
    ...
    public function tmpSave(Request $req)
    {
        ...
        try {
            ...
            if ($work_time == null) {
                ...
                $work_time['started_at'] = MailController::convertUserTimezoneToUtc0($work_time['started_at']);
                $work_time['finished_at'] = MailController::convertUserTimezoneToUtc0($work_time['finished_at']);
            }
            // 最新の作業履歴を取得する
     ...
     * @param int|null $parent_id 構成アイテムの親ID(このパラメーターが指定されている場合、親IDに含まれているサブアイテムを照会し、それがnullの場合は最上位アイテムを照会します。)
     */
    private function initJsonFromDb(&$json_array, $step_id, $parent_id)
    {
        $item_config_array = self::getItemConfigFromDb($step_id, $parent_id);
     ...
     * @param int|null $parent_id 親作業ID(このパラメーターが指定されている場合、親IDに含まれているサブアイテムを照会し、それがnullの場合は最上位アイテムを照会します。)
     * @return array 作業項目設定
     */
    private function getItemConfigFromDb($step_id, $parent_id): array
    {
        $sql = ' select item_configs.id, ' .
            ...
            ' and use_screen>> ? & 1';
        if ($parent_id === null) {
            $sql = $sql . ' and item_configs.step_id = ?  and item_configs.parent_id is null and item_configs.sort > 0';
            $params = [\Config::get('const.PROCESS_TYPE.WORK'), $step_id];
        } else {
            $sql = $sql . ' and item_configs.parent_id = ? ';
            $params = [\Config::get('const.PROCESS_TYPE.WORK'), $parent_id];
        }
