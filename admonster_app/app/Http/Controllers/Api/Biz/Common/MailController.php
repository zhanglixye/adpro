...
class MailController extends BaseController
{
    ...
    /** @var string 客户端上传的作业总时间在请求报文中的key路径(相对于MAIL_CLIENT_TIME_MAIN) */
    public $MAIL_CLIENT_TIME_REMARK;
    ...
    public function __construct(Request $req)
    {
        ...
        $this->MAIL_CLIENT_TIME_REMARK = \Config::get($mail_config_root . '.MAIL_CLIENT_TIME_REMARK');
    ...
    public function tmpSaveMail(Request $req)
    {
        try {
            ...
            $work_time['started_at'] = MailController::convertUserTimezoneToUtc0($this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_START_AT])));
            $work_time['finished_at'] = MailController::convertUserTimezoneToUtc0($this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_FINISHED_AT])));
            $work_time['total'] = $this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_TOTAL]));
            $work_time['remark'] = $this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_REMARK]));
            // 增量处理用户提交的附件
...
//                }
            ...
            // 作業時間
            $work_time = ['started_at' => null, 'finished_at' => null, 'total' => null];
            $work_time['started_at'] = self::convertUserTimezoneToUtc0($this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_START_AT])));
            $work_time['finished_at'] = self::convertUserTimezoneToUtc0($this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_FINISHED_AT])));
            $work_time['total'] = $this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_TOTAL]));
            $work_time['remark'] = $this->arrayIfNull($data, array_merge($this->MAIL_CLIENT_TIME_MAIN, [$this->MAIL_CLIENT_TIME_REMARK]));
            // 增量处理用户提交的附件
            ...
            foreach ($final_file_po_array as $item) {
                $file_size =  $item->size;
                $file_size_count = $file_size_count+$file_size;
            }
            //$file_size_count = $file_size_count+8000000;
            if ($file_size_count>7654604) {
                return self::error('添付ファイルの容量サイズはオーバーしています。ご確認ください。');
            }
    ...
    {
        // 作業時間
        $work_time = ['started_at' => null, 'finished_at' => null, 'work_time' => null, 'created_at' => null];
        // 最新の作業履歴 中间部分
        $task_result_info = TaskResult::where('task_id', $this->task_id)
            ->orderBy('id', 'asc')
            ->first();
        $task_result_info_last = TaskResult::where('task_id', $this->task_id)
            ->orderBy('id', 'desc')
            ->first();
        //获取用户时区
        //$user = Auth::user()->timezone;
        if ($task_result_info->id == $task_result_info_last->id) {
            $work_time['started_at'] = self::convertUtc0ToUserTimezone($task_result_info->created_at);
            $work_time['finished_at'] = null;
            $work_time['work_time'] = null;
        } else {
            $work_time['started_at'] = self::convertUtc0ToUserTimezone($task_result_info_last->started_at);
            $work_time['finished_at'] = self::convertUtc0ToUserTimezone($task_result_info_last->finished_at);
            $work_time['work_time'] = $task_result_info_last->work_time;
        }
        $work_time['work_time_comment'] = $task_result_info_last->work_time_comment;
        // 最新の作業履歴 右上部分
        if ($task_result_info !== null) {
            $date_user_tz = new stdClass();
            $date_user_tz->date = self::convertUtc0ToUserTimezone($task_result_info->created_at);
            $work_time['created_at'] = $date_user_tz;
        }
...
//        Uploader::tryUploadAndSave($file_contents, $file_path, 'App\Models\TaskResultFile', ['seq_no' => $max_seq_no, 'task_result_id' => $taskResult->id]);
...
//            ->where('updated_at', '<', $this->task_started_at)
    ...
    public function taskSave(array $content, array $task_result_file_po_array, array $work_time = null)
    {
        ...
        $task_result->work_time_comment = is_null($work_time)? null : $work_time['remark'];
            ...
            if (!empty($fileArray)) {
                ...
                foreach ($fileArray as $file) {
                    ...
                    if ($fileSeqNo !== null) {
                        ...
                        $file_result = self::uploadFileDetail($upload_data, 'base64', true);

                        //生成，增加到新文件数组
                        $max_seq_no++;
                        $task_result_file_po = new TaskResultFile;
                        $task_result_file_po->seq_no = $max_seq_no;
                        $task_result_file_po->name = $file_result['file_name'];
                        $task_result_file_po->file_path = $file_result['file_path'];
                        $task_result_file_po->size = $file_result['file_size'];
                        array_push($finalFileArray, $task_result_file_po);
                    }
                }
            }
     ...
     */
    public static function dynamicMeter(int $index)
    {
        $index = $index . '';
        $result_array = [];
        //for($J = 0; $J<2 ;$J++){
        // 先转换26进制
        $num26 = base_convert($index, 10, 26);
    ...
    /**
     * convert date from user timezone to utc0 .('Y-m-d H:i:s')
     * @param string|null $date date by user timezone
     * @return string|null date by utc0
     */
    ...
    /**
     * convert date from utc0 to user timezone.('Y-m-d H:i:s')
     * @param string|null $date date by utc0
     * @return string|null string by user timezone
     */
