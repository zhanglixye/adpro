...
class ImportedFilesController extends Controller
{
    ...
    public function orderStore(Request $req): JsonResponse
    {
        ...
        try {
            ...
            $s3_file_path = $req->input('file_path') ?? '';

            \DB::beginTransaction();

            $disk = \Storage::disk(\Config::get('filesystems.cloud'));
            $file_path = explode('/', $s3_file_path);
            array_shift($file_path);
            $new_s3_file_path = 'order_files/'. implode('/', $file_path);
            $disk->move($s3_file_path, $new_s3_file_path);
            // OrderFileに登録
            $order_file = new OrderFile;
            $order_file->name = end($file_path);
            $order_file->file_path = $new_s3_file_path;
            $order_file->size = $disk->exists($new_s3_file_path) ? $disk->size($new_s3_file_path) : null;
            $order_file->created_at = Carbon::now();
            $order_file->created_user_id = $user->id;
            $order_file->updated_at = Carbon::now();
            $order_file->updated_user_id = $user->id;
            $order_file->save();
            ...
            foreach ($items as $key => $item) {
                ...
                $display_format = (array_key_exists($key, $setting_rules) &&
                    ...
                    array_sum($setting_rules[$key]['displayRule']) : 0;
    ...
    public function getSettingInformation(Request $req): JsonResponse
    {
        try {
            ...
            $setting_rules = OrderFileImportColumnConfig::select('rule', 'display_format', 'item_type')
                ->where('order_file_import_main_config_id', '=', $order_file_import_main_config_id)
                ->get();
            foreach ($setting_rules as $value) {
                $value->display_format = OrderFileImportColumnConfig::getTargetDisplayFormat($value->display_format, $value->item_type);
            }
