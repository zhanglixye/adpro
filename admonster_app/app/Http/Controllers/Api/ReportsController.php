...
class ReportsController extends Controller
{
    ...
    public function getReportInfo(): JsonResponse
    {
        ...
        $steps = \DB::table('steps')
            ...
            ->orderBy('business_flows.business_id')
            ->orderBy('steps.id')
            ->get();
    ...
    private function businessReportToExcel(array $res)
    {
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $worksheet->setCellValue(
                'A' . $index,
                Carbon::parse($res[$i]->actual_date)->format('Y/m/d')
            );
            $worksheet->setCellValue(
                'B' . $index,
                $res[$i]->business_name
            ...
            $worksheet->setCellValue(
                'G' . $index,
                sprintf("%01.2f", $res[$i]->system_work_time)
            );
            $worksheet->setCellValue(
                'H' . $index,
                sprintf("%01.2f", $res[$i]->manual_work_time * 60)  // TODO: ADPORTER_PF-712
            );
            $worksheet->setCellValue(
                'I' . $index,
                $res[$i]->education_flg === \Config::get('const.FLG.ACTIVE') ? '○' : ''
            );
