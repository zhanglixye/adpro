...
class ApprovalsController extends Controller
{
    ...
    public function edit(Request $req)
    {
        ...
        $ids_for_about_step = ItemConfig::select(DB::raw('MAX(id) as id'))
            ...
            ->whereRaw('use_screen>> ? & 1', \Config::get('const.PROCESS_TYPE.APPROVAL'))  // BIT演算
    ...
    public function store(Request $req)
    {
        ...
        try {
            ...
            if ($approval_status == \Config::get('const.APPROVAL_STATUS.ON')) {
                ...
                $delivery->to_order_detail = Task::find($approval_task->task_id)->to_order_detail;
    ...
    public function againTask(Request $req)
    {
        ...
        try {
            ...
            /** @var null|int */
            $order_detail_id = \DB::table('order_details_requests')
                ->where('request_id', $request_work->request_id)
                ->value('order_detail_id');

            $step_to_order_detail = \DB::table('steps')
                ->where('id', $request_work->step_id)
                ->value('to_order_detail');

            /** @var null|array */
            $task_to_order_detail = null;
            if (!is_null($order_detail_id) && !is_null($step_to_order_detail)) {
                // tasks.to_order_detailに登録する値を作成
                $task_to_order_detail = json_decode($step_to_order_detail, true);
                $updated_order_details = [];
                foreach ($task_to_order_detail['order_details'] as $order_detail) {
                    $order_detail['id'] = $order_detail_id;
                    array_push($updated_order_details, $order_detail);
                }
                $task_to_order_detail['order_details'] = $updated_order_details;
            }
            ...
            $newTask->fill([
                ...
                'to_order_detail' => is_null($task_to_order_detail) ? $task_to_order_detail : json_encode($task_to_order_detail),
